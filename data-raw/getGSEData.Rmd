...
```{r readSampDesc1,cache=T, cach.vars=c('data_processing_frm', 'sampDesc1'), eval=T, echo=T, fig.height=6, fig.width=11}
 ...
 # Data Processing
 data_processing_frm <-  t(pData(phenoData(GSEMatrix_obj))[1,,drop=F] %>%
  dplyr::select(starts_with('data_processing')))
 data_processing_frm[,1] <- sub('data_processing.','',data_processing_frm[,1] )

 knitr::kable(data_processing_frm)%>%
  kableExtra::kable_styling(full_width = F)
...
```{r save-genes_annot, cache=T, cache.vars=''}
