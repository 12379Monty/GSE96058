...
#### CLEAR CACHE
 ...
 # Data Processing
 data_processing <-  t(pData(phenoData(GSEMatrix_obj))[1,,drop=F] %>%
...
```{r  get-counts, eval=T, cache=T, cache.vars=c('featureCount', 'genes_annot'),message=F}
 ####,results='asis'}
 ...
 featureCount <- counts_frm[, -1]
 rownames(featureCount) <- counts_frm[,1]

 genes_annot <- genes_annot[match(rownames(featureCount), genes_annot$geneSymbol),]
 names(genes_annot) <-
...
```{r save, cache=T, cache.vars='',eval=F}
 # THIS DOESNT WORK
 ...
 repl2_ndx <- grep('repl$', colnames(featureCount))
 featureCount_repl2 <- featureCount[, repl2_ndx]
 featureCount <- featureCount[, -repl2_ndx]
 rm(repl2_ndx)

 repl1_ndx <- match(sub('repl$', '', colnames(featureCount_repl2)),
   colnames(featureCount))
 featureCount_repl1 <- featureCount[, repl1_ndx]
 featureCount <- featureCount[, -repl1_ndx]
 rm(repl1_ndx)

 # save repl counts
 save(list='featureCount_repl1', file=file.path(data_DIR, 'featureCount_repl1.rda'),
   compress='xz')
 save(list='featureCount_repl2', file=file.path(data_DIR, 'featureCount_repl2.rda'),
   compress='xz')


 # divide the rest into 5 groups - 
 sample_ndx <- match(colnames(featureCount), sampDesc$title)
 ...
 for(JJ in 1:length(fold_samples_lst)){
  sub_ndx <- match(fold_samples_lst[[JJ]], colnames(featureCount))
  assign(paste0('featureCount_sub',JJ), featureCount[, sub_ndx])
  featureCount <- featureCount[, -sub_ndx]

   save(list=paste0('featureCount_sub',JJ,'.rda'), 
   file=file.path(data_DIR, paste0('featureCount_sub',JJ)),
   compress='xz')

 } 


 if(ncol(featureCount)) stop("Oh, oh - featureCount should now be empty!!!!")


  
```
 
```{r save-genes_annot, cache=T, cache.vars='',eval=F}
 # should be saved with the rest but forgot...
...
After thought ... add featureCount table membership to sampDesc
```{r , include=F}
library(GSE96058)
```

```{r add-countData-to-sampDesc, cache=T, cache.vars=''}

sample_feature_table_vec <- with(sampDesc,
ifelse(title %in% colnames(featureCount_repl1), 'repl1',
ifelse(title %in% colnames(featureCount_repl2), 'repl2',
ifelse(title %in% colnames(featureCount_sub1), 'sub1',
ifelse(title %in% colnames(featureCount_sub2), 'sub2',
ifelse(title %in% colnames(featureCount_sub3), 'sub3',
ifelse(title %in% colnames(featureCount_sub4), 'sub4',
ifelse(title %in% colnames(featureCount_sub5), 'sub5',
'ERROR'))))))))

table(sample_feature_table_vec)

sampDesc <- data.frame(countTable=sample_feature_table_vec, sampDesc)
